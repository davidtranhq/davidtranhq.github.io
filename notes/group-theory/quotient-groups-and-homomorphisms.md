...
* $\sigma \in S_n$ is an **even permutation** if it has an even number of transpositions, and is an **odd permutation** otherwise.
* The alternating group $A_n \leq S_n$ is the set of even permutations of $S_n$.
* The product of two permutations are even if and only if they have the same parity (even or odd).
* It is easy to see that an $m$-cycle is an odd permutation if and only if $m$ is even, and thus the permutation of $\sigma$ is odd if and only if the number of cycles of even length in its cycle decomposition is odd.

### Exercises

**Exercise 3**: Prove that $S_n$ is generated by $\lbrace (i,i+1) \mid 1 \leq i \leq n - 1 \rbrace$.

*Proof*: We know that $S_n$ may be generated by any transposition. It suffices to show that the adjacent transpositions generate the set of transpositions. It is straightforward to show by induction that $(1, k) = (k,k-1)(1,k-1)(k,k-1)^{-1}$ for all $1 \leq k \leq n$. $\blacksquare$

**Exercise 4**: Show that $S_n = \langle (1,2), (1,2,3,\dots,n) \rangle$

*Proof*: $(1,2,\dots,n)$ can be represented as a product of transpositions, from which the result follows from Exercise 3. $\blacksquare$

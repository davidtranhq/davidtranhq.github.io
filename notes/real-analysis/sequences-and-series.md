...
*Proof*: Suppose first that $\lim_\infty a_n = \lim_\infty c_n = L \in \mathbb R$ and let $N_1 \in \mathbb N$ such that $a_n \leq b_n \leq c_n, \forall n \geq N_1$. TODO.
...
*Proof*: Note that the sequence of partial sums is strictly increasing since $a_n > 0$. We claim that it suffices to find a subsequence $(s_{n_k})_{k=0}^\infty$ of $(s_n)_{n=1}^\infty$ such that $\lim_{k \to \infty} s_{n_k} = \infty$ (**TODO**). Consider $(s_{2^k})_{k = 0}^\infty$. Observe 
$$
\begin{align*}
s_1 &= 1 \\ 
s_2 &= 1 + 1/2 \\
